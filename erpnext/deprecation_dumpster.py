...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the VantageSuite app. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The VantageSuite version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
### Party starts here
@deprecated(
	"VantageSuite.controllers.taxes_and_totals.get_itemised_taxable_amount",
	"2024-11-07",
	"v17",
...
@deprecated(
	"VantageSuite.stock.get_pos_profile_item_details",
	"2024-11-19",
	"v16",
	"Use VantageSuite.stock.get_pos_profile_item_details_ with a flipped signature",
)
def get_pos_profile_item_details(company, ctx, pos_profile=None, update_data=False):
	from VantageSuite.stock.get_item_details import get_pos_profile_item_details_

	return get_pos_profile_item_details_(ctx, company, pos_profile=pos_profile, update_data=update_data)


@deprecated(
	"VantageSuite.stock.get_item_warehouse",
	"2024-11-19",
	"v16",
	"Use VantageSuite.stock.get_item_warehouse_ with a flipped signature",
)
def get_item_warehouse(item, ctx, overwrite_warehouse, defaults=None):
	from VantageSuite.stock.get_item_details import get_item_warehouse_
