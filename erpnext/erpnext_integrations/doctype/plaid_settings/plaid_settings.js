...
frappe.provide("VantageSuite.integrations");
	...
	refresh: function (frm) {
		if (frm.doc.enabled) {
			frm.add_custom_button(__("Link a new bank account"), () => {
				new VantageSuite.integrations.plaidLink(frm);
			});
			...
			frm.add_custom_button(__("Sync Now"), () => {
				frappe.call({
					method: "VantageSuite.erpnext_integrations.doctype.plaid_settings.plaid_settings.enqueue_synchronization",
					freeze: true,
					callback: () => {
...
VantageSuite.integrations.plaidLink = class plaidLink {
	constructor(parent) {
		this.frm = parent;
	...
	plaid_success(token, response) {
		...
		frappe.prompt(
			...
			(data) => {
				...
				frappe
					.xcall(
						"VantageSuite.erpnext_integrations.doctype.plaid_settings.plaid_settings.add_institution",
						{
							token: token,
							response: response,
						}
					...
					.then((result) => {
						frappe.xcall(
							"VantageSuite.erpnext_integrations.doctype.plaid_settings.plaid_settings.add_bank_accounts",
							{
								response: response,
								bank: result,
								company: me.company,
