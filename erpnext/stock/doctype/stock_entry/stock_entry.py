...
import vantagesuite-e
from vantagesuite-e.accounts.general_ledger import process_gl_map
from vantagesuite-e.accounts.utils import get_account_currency
from vantagesuite-e.buying.utils import check_on_hold_or_closed_status
from vantagesuite-e.controllers.taxes_and_totals import init_landed_taxes_and_totals
from vantagesuite-e.manufacturing.doctype.bom.bom import (
	add_additional_cost,
...
from vantagesuite-e.setup.doctype.brand.brand import get_brand_defaults
from vantagesuite-e.setup.doctype.item_group.item_group import get_item_group_defaults
from vantagesuite-e.stock.doctype.batch.batch import get_batch_qty
from vantagesuite-e.stock.doctype.item.item import get_item_defaults
from vantagesuite-e.stock.doctype.serial_no.serial_no import get_serial_nos
from vantagesuite-e.stock.doctype.stock_reconciliation.stock_reconciliation import (
	OpeningEntryAccountError,
)
from vantagesuite-e.stock.get_item_details import (
	ItemDetailsCtx,
	get_barcode_data,
	get_bin_details,
...
from vantagesuite-e.stock.serial_batch_bundle import (
	SerialBatchCreation,
	get_empty_batches_based_work_order,
	get_serial_or_batch_items,
)
from vantagesuite-e.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from vantagesuite-e.stock.utils import get_bin, get_combine_datetime, get_incoming_rate
...
from vantagesuite-e.controllers.stock_controller import StockController
from vantagesuite-e.controllers.subcontracting_inward_controller import SubcontractingInwardController
...
class StockEntry(StockController, SubcontractingInwardController):
	...
	if TYPE_CHECKING:
		...
		from vantagesuite-e.stock.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from vantagesuite-e.stock.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def before_validate(self):
		from vantagesuite-e.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate_difference_account(self):
		if not cint(vantagesuite-e.is_perpetual_inventory_enabled(self.company)):
			return
	...
	def set_actual_qty(self):
		from vantagesuite-e.stock.stock_ledger import is_negative_stock_allowed
	...
	def validate_same_source_target_warehouse_during_material_transfer(self):
		...
		if frappe.get_single_value("Stock Settings", "validate_material_transfer_warehouses"):
			from vantagesuite-e.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				...
				d.basic_rate = get_valuation_rate(
					...
					currency=vantagesuite-e.get_company_currency(self.company),
					company=self.company,
	...
	def get_bom_raw_materials(self, qty):
		from vantagesuite-e.manufacturing.doctype.bom.bom import get_bom_items_as_dict
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from vantagesuite-e.subcontracting.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from vantagesuite-e.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def move_sample_to_retention_warehouse(company, items):
	from vantagesuite-e.stock.doctype.serial_and_batch_bundle.test_serial_and_batch_bundle import (
		get_batch_from_bundle,
	)
	from vantagesuite-e.stock.serial_batch_bundle import SerialBatchCreation
...
def get_expired_batch_items():
	from vantagesuite-e.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
...
def get_items_from_subcontract_order(source_name, target_doc=None):
	from vantagesuite-e.controllers.subcontracting_controller import make_rm_stock_entry
...
def get_stock_entry_data(work_order, stock_entry_doc=None):
	from vantagesuite-e.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import (
