...
frappe.ui.form.on("Batch", {
	...
	make_dashboard: (frm) => {
		if (!frm.is_new()) {
			...
			frappe.call({
				method: "vantagesuite-e.stock.doctype.batch.batch.get_batch_qty",
				args: {
					batch_no: frm.doc.name,
				...
				callback: (r) => {
					...
					rows.find(".btn-move").on("click", function () {
						...
						frappe.prompt(
							...
							(data) => {
								frappe.call({
									method: "vantagesuite-e.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry",
									args: {
										item_code: frm.doc.item,
										batch_no: frm.doc.name,
					...
					rows.find(".btn-split").on("click", function () {
						...
						frappe.prompt(
							...
							(data) => {
								frappe
									.xcall("vantagesuite-e.stock.doctype.batch.batch.split_batch", {
										item_code: frm.doc.item,
										batch_no: frm.doc.name,
										qty: data.qty,
