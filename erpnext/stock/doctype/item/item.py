...
from vantagesuite-e.controllers.item_variant import (
...
from vantagesuite-e.stock.doctype.item_default.item_default import ItemDefault
from vantagesuite-e.stock.utils import get_valuation_method
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from vantagesuite-e.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from vantagesuite-e.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from vantagesuite-e.stock.doctype.item_default.item_default import ItemDefault
		from vantagesuite-e.stock.doctype.item_reorder.item_reorder import ItemReorder
		from vantagesuite-e.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from vantagesuite-e.stock.doctype.item_tax.item_tax import ItemTax
		from vantagesuite-e.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from vantagesuite-e.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": vantagesuite-e.get_default_currency(),
	...
	def set_opening_stock(self):
		...
		from vantagesuite-e.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
	...
	def recalculate_bin_qty(self, new_name):
		from vantagesuite-e.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"vantagesuite-e.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=frappe.in_test,
						timeout=600,
...
def convert_erpnext_to_barcodenumber(vantagesuite-e_number, barcode):
	if vantagesuite-e_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if vantagesuite-e_number in convert:
		return convert[vantagesuite-e_number]
...
def get_asset_naming_series():
	from vantagesuite-e.assets.doctype.asset.asset import get_asset_naming_series
