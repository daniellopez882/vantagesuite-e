...
frappe.provide("vantagesuite-e.item");
...
frappe.ui.form.on("Item", {
	...
	onload: function (frm) {
		vantagesuite-e.item.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						vantagesuite-e.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						vantagesuite-e.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						vantagesuite-e.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		vantagesuite-e.item.edit_prices_button(frm);
		vantagesuite-e.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			vantagesuite-e.item.make_dashboard(frm);
		}
	...
	validate: function (frm) {
		vantagesuite-e.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "vantagesuite-e.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
$.extend(vantagesuite-e.item, {
	setup_queries: function (frm) {
		...
		frm.fields_dict["item_defaults"].grid.get_field("income_account").get_query = function (
			...
			const row = locals[cdt][cdn];
			return {
				query: "vantagesuite-e.controllers.queries.get_income_account",
				filters: { company: row.company },
			};
		};
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse_group").get_query = function (
			...
			let row = locals[cdt][cdn];
			return {
				query: "vantagesuite-e.stock.doctype.warehouse.warehouse.get_warehouses_for_reorder",
				filters: {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				vantagesuite-e.item.item_dashboard = new vantagesuite-e.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "vantagesuite-e.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				vantagesuite-e.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "vantagesuite-e.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "vantagesuite-e.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "vantagesuite-e.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "vantagesuite-e.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "vantagesuite-e.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
...
frappe.tour["Item"] = [
	...
	},
	{
		...
		description: __(
			"If you are maintaining stock of this Item in your Inventory, vantagesuite-e will make a stock ledger entry for each transaction of this item."
		),
	},
	{
