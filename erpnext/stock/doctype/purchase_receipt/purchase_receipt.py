...
from vantagesuite-e.controllers.accounts_controller import merge_taxes
from vantagesuite-e.controllers.buying_controller import BuyingController
from vantagesuite-e.stock.doctype.delivery_note.delivery_note import make_inter_company_transaction
from vantagesuite-e.stock.doctype.stock_reservation_entry.stock_reservation_entry import StockReservation
...
class PurchaseReceipt(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from vantagesuite-e.accounts.doctype.item_wise_tax_detail.item_wise_tax_detail import ItemWiseTaxDetail
		from vantagesuite-e.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from vantagesuite-e.accounts.doctype.purchase_taxes_and_charges.purchase_taxes_and_charges import (
			PurchaseTaxesandCharges,
		)
		from vantagesuite-e.buying.doctype.purchase_receipt_item_supplied.purchase_receipt_item_supplied import (
			PurchaseReceiptItemSupplied,
		)
		from vantagesuite-e.stock.doctype.purchase_receipt_item.purchase_receipt_item import PurchaseReceiptItem
	...
	def make_item_gl_entries(self, gl_entries, inventory_account_map=None):
		from vantagesuite-e.accounts.doctype.purchase_invoice.purchase_invoice import (
		...
		for d in self.get("items"):
			...
			if (
				...
				if not (
					(vantagesuite-e.is_perpetual_inventory_enabled(self.company) and d.item_code in stock_items)
...
def make_purchase_invoice(source_name, target_doc=None, args=None):
	...
	from vantagesuite-e.accounts.party import get_payment_terms_template
...
def make_purchase_return_against_rejected_warehouse(source_name):
	from vantagesuite-e.controllers.sales_and_purchase_return import make_return_doc
