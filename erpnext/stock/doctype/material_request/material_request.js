...
frappe.ui.form.on("Material Request", {
	setup: function (frm) {
		...
		frm.set_query("item_code", "items", function () {
			return {
				query: "vantagesuite-e.controllers.queries.item_query",
			};
		});
	...
	onload: function (frm) {
		...
		vantagesuite-e.utils.add_item(frm);
	...
	company: function (frm) {
		...
		vantagesuite-e.utils.set_letter_head(frm);
	},
	...
	update_status: function (frm, stop_status) {
		frappe.call({
			method: "vantagesuite-e.stock.doctype.material_request.material_request.update_status",
	...
	get_items_from_sales_order: function (frm) {
		vantagesuite-e.utils.map_current_doc({
			method: "vantagesuite-e.selling.doctype.sales_order.sales_order.make_material_request",
	...
	get_item_data: function (frm, item, overwrite_warehouse = false) {
		...
		frappe.call({
			method: "vantagesuite-e.stock.get_item_details.get_item_details",
			args: {
				ctx: {
		...
		frappe.prompt(
			{
				...
				get_query: () => {
					return {
						query: "vantagesuite-e.stock.doctype.material_request.material_request.get_default_supplier_query",
			...
			(values) => {
				frappe.model.open_mapped_doc({
					method: "vantagesuite-e.stock.doctype.material_request.material_request.make_purchase_order",
					frm: frm,
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.material_request.material_request.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true,
		});
	},
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.material_request.material_request.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_in_transit_stock_entry(frm) {
		frappe.prompt(
			...
			(values) => {
				frappe.call({
					method: "vantagesuite-e.stock.doctype.material_request.material_request.make_in_transit_stock_entry",
					args: {
	...
	create_pick_list: (frm) => {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.material_request.material_request.create_pick_list",
			frm: frm,
		});
	},
	...
	raise_work_orders: function (frm) {
		frappe.call({
			method: "vantagesuite-e.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
frappe.ui.form.on("Material Request Item", {
	...
	schedule_date: function (frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				vantagesuite-e.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
vantagesuite-e.buying.MaterialRequestController = class MaterialRequestController extends (
...
extend_cscript(cur_frm.cscript, new vantagesuite-e.buying.MaterialRequestController({ frm: cur_frm }));

function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		vantagesuite-e.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
