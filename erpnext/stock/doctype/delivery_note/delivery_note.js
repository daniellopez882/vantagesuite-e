...
frappe.provide("vantagesuite-e.stock.delivery_note");
frappe.provide("vantagesuite-e.accounts.dimensions");

vantagesuite-e.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
vantagesuite-e.accounts.taxes.setup_tax_validations("Delivery Note");
vantagesuite-e.sales_common.setup_selling_controller();
...
frappe.ui.form.on("Delivery Note", {
	setup: function (frm) {
		...
		vantagesuite-e.queries.setup_queries(frm, "Warehouse", function () {
			return vantagesuite-e.queries.warehouse(frm.doc);
		});
		vantagesuite-e.queries.setup_warehouse_query(frm);
		...
		frm.set_query("expense_account", "items", function (doc, cdt, cdn) {
			if (vantagesuite-e.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function (frm) {
		vantagesuite-e.stock.delivery_note.set_print_hide(frm.doc);
	},
			...
			frm.add_custom_button(
				...
				function () {
					frappe.model.open_mapped_doc({
						method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
						frm: cur_frm,
					});
				},
				__("Create")
		...
		if (
			...
			if (internal) {
				...
				frm.add_custom_button(
					...
					function () {
						frappe.model.open_mapped_doc({
							method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt",
							frm: frm,
						});
					},
					__("Create")
...
vantagesuite-e.stock.DeliveryNoteController = class DeliveryNoteController extends (
	vantagesuite-e.selling.SellingController
) {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
			...
			this.frm.add_custom_button(
				...
				function () {
					...
					vantagesuite-e.utils.map_current_doc({
						method: "vantagesuite-e.selling.doctype.sales_order.sales_order.make_delivery_note",
						args: {
		...
		if (
			...
			this.frm.add_custom_button(
				...
				function () {
					...
					vantagesuite-e.utils.map_current_doc({
						method: "vantagesuite-e.stock.doctype.pick_list.pick_list.create_dn_for_pick_lists",
						...
						get_query_method: "vantagesuite-e.stock.doctype.pick_list.pick_list.get_pick_list_query",
						size: "extra-large",
						allow_child_item_selection: true,
		...
		if (!doc.is_return && doc.status != "Closed") {
			...
			if (
				...
				this.frm.add_custom_button(
					...
					function () {
						frappe.model.open_mapped_doc({
							method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.make_packing_slip",
							frm: me.frm,
						});
					},
					__("Create")
		...
		vantagesuite-e.stock.delivery_note.set_print_hide(doc, dt, dn);
	}

	make_shipment() {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm,
		});
	}
	...
	make_installation_note() {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.make_installation_note",
			frm: this.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm,
		});
	}
	...
	make_delivery_trip() {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm,
		});
	}
	...
	update_status(status) {
		...
		frappe.call({
			method: "vantagesuite-e.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new vantagesuite-e.stock.DeliveryNoteController({ frm: cur_frm }));
...
frappe.ui.form.on("Delivery Note", {
	...
	unhide_account_head: function (frm) {
		...
		var aii_enabled = vantagesuite-e.is_perpetual_inventory_enabled(frm.doc.company);
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	},
});

vantagesuite-e.stock.delivery_note.set_print_hide = function (doc, cdt, cdn) {
