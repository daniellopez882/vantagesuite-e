...
frappe.ui.form.on("Pick List", {
	...
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "vantagesuite-e.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
					company: frm.doc.company,
				},
			};
				...
				frappe.prompt(
					...
					(data) => {
						...
						vantagesuite-e.utils.map_current_doc({
							method: "vantagesuite-e.manufacturing.doctype.work_order.work_order.create_pick_list",
							target: frm,
	...
	create_delivery_note: (frm) => {
		frappe.model.open_mapped_doc({
			method: "vantagesuite-e.stock.doctype.pick_list.pick_list.create_delivery_note",
			frm: frm,
		});
	},
	create_stock_entry: (frm) => {
		frappe
			.xcall("vantagesuite-e.stock.doctype.pick_list.pick_list.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
	...
	add_get_items_button: (frm) => {
		...
		frm.get_items_btn = frm.add_custom_button(__("Get Items"), () => {
			vantagesuite-e.utils.map_current_doc({
				method: "vantagesuite-e.selling.doctype.sales_order.sales_order.create_pick_list",
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new vantagesuite-e.utils.BarcodeScanner(opts);
...
function get_item_details(item_code, uom = null, warehouse = null, company = null) {
	if (item_code) {
		return frappe.xcall("vantagesuite-e.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
			warehouse,
			company,
