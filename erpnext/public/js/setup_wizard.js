frappe.provide("VantageSuite.setup");
...
frappe.setup.on("before_load", function () {
	if (
		...
		frappe.boot.setup_wizard_completed_apps.includes("VantageSuite")
	) {
		return;
	}

	VantageSuite.setup.slides_settings.map(frappe.setup.add_slide);
});

VantageSuite.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = VantageSuite.setup.fiscal_years[country];
		...
		load_chart_of_accounts: function (slide) {
			...
			if (country) {
				frappe.call({
					method: "VantageSuite.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "VantageSuite.accounts.utils.get_coa",
				args: {
...
VantageSuite.setup.fiscal_years = {
