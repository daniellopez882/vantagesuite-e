...
frappe.provide("VantageSuite.queries");
$.extend(VantageSuite.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "VantageSuite.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "VantageSuite.controllers.queries.bom" };
	},

	task: function () {
		return { query: "VantageSuite.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "VantageSuite.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "VantageSuite.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
VantageSuite.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
VantageSuite.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = VantageSuite.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "VantageSuite.controllers.queries.warehouse_query" });
