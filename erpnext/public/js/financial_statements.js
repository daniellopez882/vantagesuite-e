frappe.provide("VantageSuite.financial_statements");

VantageSuite.financial_statements = {
	filters: get_filters(),
	baseData: null,
	...
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		// Growth/Margin
		if (VantageSuite.financial_statements._is_special_view(column, data))
			return VantageSuite.financial_statements._format_special_view(...report_params);

		if (frappe.query_report.get_filter_value("report_template"))
			return VantageSuite.financial_statements._format_custom_report(...report_params);
		else return VantageSuite.financial_statements._format_standard_report(...report_params);
	},
	...
	_format_custom_report: function (value, row, column, data, default_formatter, filter) {
		const columnInfo = VantageSuite.financial_statements._parse_column_info(column.fieldname, data);
		const formatting = VantageSuite.financial_statements._get_formatting_for_column(data, columnInfo);

		if (columnInfo.isAccount) {
			return VantageSuite.financial_statements._format_custom_account_column(
				value,
				data,
				formatting,
				column,
			...
			return VantageSuite.financial_statements._format_custom_value_column(
				value,
				data,
				formatting,
				column,
	...
	_parse_column_info: function (fieldname, data) {
		...
		return {
			isAccount: baseName === VantageSuite.financial_statements.name_field,
	...
	_format_custom_account_column: function (value, data, formatting, column, default_formatter, row) {
		...
		if (should_link_to_ledger) {
			...
			column.link_onclick =
				"VantageSuite.financial_statements.open_general_ledger(" + JSON.stringify(glData) + ")";
		...
		// Style
		return VantageSuite.financial_statements._style_custom_value(formattedValue, formatting, null);
	},
	...
	_format_custom_value_column: function (value, data, formatting, column, default_formatter, row) {
		...
		return VantageSuite.financial_statements._style_custom_value(formattedValue, formatting, value);
	},
	...
	_format_standard_report: function (value, row, column, data, default_formatter, filter) {
		if (data && column.fieldname == VantageSuite.financial_statements.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"VantageSuite.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		VantageSuite.financial_statements.filters = get_filters();

		let fiscal_year = VantageSuite.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: VantageSuite.get_presentation_currency_list(),
		},
		{
	...
	let fiscal_year = VantageSuite.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
	if (fiscal_year) {
