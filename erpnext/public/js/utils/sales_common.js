...
frappe.provide("VantageSuite.selling");

VantageSuite.sales_common = {
	setup_selling_controller: function () {
		VantageSuite.selling.SellingController = class SellingController extends VantageSuite.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], VantageSuite.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", VantageSuite.queries.contact_query);
				me.frm.set_query("company_contact_person", VantageSuite.queries.company_contact_query);
				me.frm.set_query("customer_address", VantageSuite.queries.address_query);
				me.frm.set_query("shipping_address_name", VantageSuite.queries.address_query);
				me.frm.set_query("dispatch_address_name", VantageSuite.queries.dispatch_address_query);
				me.frm.set_query("company_address", VantageSuite.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "VantageSuite.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				VantageSuite.utils.get_address_display(this.frm, "customer_address");
			...
			shipping_address_name() {
				VantageSuite.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				VantageSuite.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				VantageSuite.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			project() {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "VantageSuite.projects.doctype.project.project.get_cost_center_name",
