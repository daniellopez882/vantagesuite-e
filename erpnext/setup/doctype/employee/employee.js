...
frappe.provide("vantagesuite-e.setup");
vantagesuite-e.setup.EmployeeController = class EmployeeController extends frappe.ui.form.Controller {
	setup() {
		...
		this.frm.fields_dict.reports_to.get_query = function (doc, cdt, cdn) {
			return {
				query: "vantagesuite-e.controllers.queries.employee_query",
				filters: [
					["status", "=", "Active"],
					["name", "!=", doc.name],
				],
...
frappe.ui.form.on("Employee", {
	setup: function (frm) {
		frm.make_methods = {
			"Bank Account": () => vantagesuite-e.utils.make_bank_account(frm.doc.doctype, frm.doc.name),
		};
	},
	...
	create_user: function (frm) {
		...
		frappe.call({
			method: "vantagesuite-e.setup.doctype.employee.employee.create_user",
			args: {
...
cur_frm.cscript = new vantagesuite-e.setup.EmployeeController({
	frm: cur_frm,
});
