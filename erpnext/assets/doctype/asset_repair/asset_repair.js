...
frappe.ui.form.on("Asset Repair", {
	setup: function (frm) {
		...
		frm.set_query("purchase_invoice", "invoices", function () {
			return {
				query: "VantageSuite.assets.doctype.asset_repair.asset_repair.get_purchase_invoice",
				filters: {
					company: frm.doc.company,
				},
			};
		...
		frm.set_query("expense_account", "invoices", function (doc, cdt, cdn) {
			...
			return {
				query: "VantageSuite.assets.doctype.asset_repair.asset_repair.get_expense_accounts",
				filters: {
	...
	repair_status: (frm) => {
		if (frm.doc.completion_date && frm.doc.repair_status == "Completed") {
			frappe.call({
				method: "VantageSuite.assets.doctype.asset_repair.asset_repair.get_downtime",
				args: {
...
frappe.ui.form.on("Asset Repair Purchase Invoice", {
	...
	expense_account: function (frm, cdt, cdn) {
		...
		frappe.call({
			method: "VantageSuite.assets.doctype.asset_repair.asset_repair.get_unallocated_repair_cost",
			args: {
				purchase_invoice: row.purchase_invoice,
	...
	pick_serial_and_batch(frm, cdt, cdn) {
		...
		frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
			if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
				...
				new VantageSuite.SerialBatchPackageSelector(frm, item, (r) => {
					if (r) {
