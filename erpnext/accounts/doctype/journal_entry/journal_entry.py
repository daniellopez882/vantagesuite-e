...
import VantageSuite
from VantageSuite.accounts.deferred_revenue import get_deferred_booking_accounts
...
from VantageSuite.accounts.doctype.tax_withholding_entry.tax_withholding_entry import JournalTaxWithholding
...
from VantageSuite.assets.doctype.asset_depreciation_schedule.asset_depreciation_schedule import (
	get_depr_schedule,
)
from VantageSuite.controllers.accounts_controller import AccountsController
from VantageSuite.setup.utils import get_exchange_rate as _get_exchange_rate
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from VantageSuite.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from VantageSuite.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": VantageSuite.get_default_cost_center(self.company),
						},
					)
...
def get_outstanding(args):
	...
	company_currency = VantageSuite.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None):
	...
	company_currency = VantageSuite.get_company_currency(company)
	account_details = frappe.get_cached_value(
