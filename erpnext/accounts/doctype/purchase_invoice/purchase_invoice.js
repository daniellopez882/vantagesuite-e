...
VantageSuite.accounts.payment_triggers.setup("Purchase Invoice");
VantageSuite.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
VantageSuite.accounts.taxes.setup_tax_validations("Purchase Invoice");
VantageSuite.buying.setup_buying_controller();

VantageSuite.accounts.PurchaseInvoice = class PurchaseInvoice extends VantageSuite.buying.BuyingController {
	setup(doc) {
	...
	refresh(doc) {
		...
		if (doc.docstatus === 0) {
			...
			this.frm.add_custom_button(
				...
				function () {
					VantageSuite.utils.map_current_doc({
						method: "VantageSuite.stock.doctype.purchase_receipt.purchase_receipt.make_purchase_invoice",
	...
	unblock_invoice() {
		const me = this;
		frappe.call({
			method: "VantageSuite.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice",
	...
	make_comment_dialog_and_block_invoice() {
		...
		this.dialog.set_primary_action(__("Save"), function () {
			...
			frappe.call({
				method: "VantageSuite.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice",
				args: {
	...
	set_release_date(data) {
		return frappe.call({
			method: "VantageSuite.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date",
			args: data,
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "VantageSuite.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "VantageSuite.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(VantageSuite.accounts.PurchaseInvoice);
...
frappe.ui.form.on("Purchase Invoice", {
	...
	mode_of_payment: function (frm) {
		VantageSuite.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
	...
	make_lcv(frm) {
		frappe.call({
			method: "VantageSuite.stock.doctype.purchase_receipt.purchase_receipt.make_lcv",
			args: {
	...
	make_purchase_receipt: function (frm) {
		frappe.model.open_mapped_doc({
			method: "VantageSuite.accounts.doctype.purchase_invoice.purchase_invoice.make_purchase_receipt",
			frm: frm,
