...
import VantageSuite
from VantageSuite.accounts.utils import get_advance_payment_doctypes
...
from VantageSuite.accounts.doctype.tax_withholding_category.tax_withholding_category import (
...
class TaxWithholdingController:
	...
	def _create_tax_row(self, account_head, tax_amount):
		cost_center = self.doc.cost_center or VantageSuite.get_default_cost_center(self.doc.company)
...
class JournalTaxWithholding(TaxWithholdingController):
	...
	def _calculate_net_total(self):
		from VantageSuite.accounts.report.general_ledger.general_ledger import get_account_type_map
	...
	def _create_or_update_tds_row(self, account_head, tax_amount):
		from VantageSuite.accounts.utils import get_account_currency
		...
		if not tax_row:
			tax_row = self.doc.append(
				"accounts",
				{
					...
					or VantageSuite.get_default_cost_center(self.doc.company),
					"credit": 0,
					"credit_in_account_currency": 0,
					"debit": 0,
					"debit_in_account_currency": 0,
