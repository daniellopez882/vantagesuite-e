// Copyright (c) 2020, Daniel Lopez and contributors
// For license information, please see license.txt

frappe.provide("VantageSuite.accounts");
VantageSuite.sales_common.setup_selling_controller();

VantageSuite.accounts.pos.setup("POS Invoice");
VantageSuite.selling.POSInvoiceController = class POSInvoiceController extends VantageSuite.selling.SellingController {
	settings = {};
	...
	onload(doc) {
		...
		this.frm.set_query("item_code", "items", function (doc) {
			return {
				query: "VantageSuite.accounts.doctype.pos_invoice.pos_invoice.item_query",
				filters: {
					has_variants: ["=", 0],
					is_sales_item: ["=", 1],
					disabled: ["=", 0],
	...
	pos_profile(frm) {
		...
		frappe.call({
			method: "VantageSuite.selling.page.point_of_sale.point_of_sale.get_pos_profile_data",
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "VantageSuite.accounts.doctype.pos_invoice.pos_invoice.make_sales_return",
			frm: this.frm,
		});
	}
...
extend_cscript(cur_frm.cscript, new VantageSuite.selling.POSInvoiceController({ frm: cur_frm }));
